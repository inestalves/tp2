"""
Componente 2: Rule Engine

Converte regras de deteccao descritas em linguagem natural (pelo gestor de
loja) para o formato JSON estruturado definido na Seccao 5.3 do enunciado, e
executa essas regras sobre inspection records (gerados por
shelf_inspector.py).

Schema de uma regra (ver prompts/rule_schema.json):
    {
      "rule_id": "RULE_001",
      "created_at": "...",
      ...
      "conditions": {
        "zone_filter": ["Z_S1","Z_S3"] | null,
        "time_filter": {"hours_start": 10, "hours_end": 13} | null,
        "issue_types": ["empty_shelf","damaged"] | null,
        "severity_threshold": "low|medium|high" | null,
        "fill_rate_threshold": 0.6 | null,
        "location_filter": "bottom|middle|top|any" | null
      },
      "action": {"alert_level": "info|warning|critical", "notification_message": "..."},
      "validation": {"is_valid": true, "ambiguities": [...], "assumptions": [...]}
    }

Uma regra dispara para um inspection record se TODAS as condicoes
preenchidas (nao-null) em "conditions" se verificarem (AND):
- "zone_filter"/"time_filter" funcionam como filtros de aplicabilidade
  (zona/horario da inspecao).
- "issue_types" (+ "severity_threshold" + "location_filter") verifica-se se
  existir pelo menos um issue do tipo indicado, com severidade >= limiar e
  localizacao compativel.
- "fill_rate_threshold" verifica-se se shelf_fill_rate < limiar.
Se nenhuma condicao estiver preenchida, a regra nunca dispara.

Uso:
    python src/rule_engine.py --add "Alertar sempre que o shelf_fill_rate for inferior a 70%"
    python src/rule_engine.py --list
    python src/rule_engine.py --delete RULE_003
    python src/rule_engine.py --run --inspection data/inspections/INS_xxx.json
